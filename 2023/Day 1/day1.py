...
last = [lines[count].replace(numstrings[x.index(max(filter(lambda a: a != None, x)))], nums[x.index(max(filter(lambda a: a != None, x)))], 1) for count, x in enumerate(indices) if any(x)]
